...
class CartesianDMP(DMPBase):
    ...
    def set_constants(self, alpha_y, beta_y):
    ...
    def step(self, last_y, last_yd, goal_y=None, coupling_term=None,
        ...
        if goal_y is None:
            goal_y = self.goal_y[:3]
        step_function(
            self.last_t, self.t,
            self.current_y[:3], self.current_yd[:3],
            goal_y, self.goal_yd[:3], self.goal_ydd[:3],
    ...
    def open_loop(self, start_y=None, goal_y=None, run_t=None, coupling_term=None,
        ...
        if start_y is None:
            start_y = self.start_y[:3]
        if goal_y is None:
            goal_y = self.goal_y[:3]
        T, Yp = dmp_open_loop(
            self.execution_time_, 0.0, self.dt_,
            self.start_y[:3], goal_y,
            self.alpha_y, self.beta_y,
            self.forcing_term_pos,
            coupling_term,
            run_t, self.int_dt,
